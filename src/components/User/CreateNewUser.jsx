...
function CreateNewUser() {
  ...
  const allSites = loginData.usersites.map(s => ({ ...s, label: s.site_name, value: s.site_id }))
  ...
  return (
    <div style={{ padding: '0 30px', background: '#fff', height: '92vh' }}>
      <Box className='inspection-title bottom-lines' style={{ marginBottom: '20px' }}>
        <div style={{ fontSize: '16px', fontWeight: 800 }}>Create User</div>
        <span className='bread-crum'>Fill the details to create the user. </span>
      </Box>
      <div>
        <div style={{ background: '#fff', borderRadius: '8px', marginBottom: '18px' }}>
          <div style={{ borderRadius: '5px', border: '1px solid #dee2e6', marginBottom: '25px', transition: `box-shadow 1s` }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '50% 50%', padding: '16px' }}>
              ...
              {/* {!selectedRole.includes('Operator') && <MinimalInput onFocus={() => handleFocus('username')} error={errors.username} placeholder='Enter Username' value={userName} onChange={setUsername} label='Username' w={98} />} */}
              {!selectedRole.includes('Operator') && <MinimalInput onFocus={() => handleFocus('email')} error={errors.email} placeholder='Enter Email' value={email} onChange={setEmail} label='Email' w={98} />}
          ...
          <div style={{ borderRadius: '5px', border: '1px solid #dee2e6', marginBottom: '16px', transition: `box-shadow 1s` }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '50% 50%', padding: '16px' }}>
              {checkUserRole.isSuperAdmin() && <MinimalAutoComplete value={company} onFocus={() => handleFocus('ac_active_company')} error={errors.ac_active_company} placeholder='Select a Company' onChange={v => handleCompany(v)} options={companyOptions} label='Select Company' w={98} />}
              ...
              {selectedRole.length > 1 && !selectedRole.includes('Operator') && <MinimalAutoComplete onFocus={() => handleFocus('ac_default_role_app')} error={errors.ac_default_role_app} placeholder='Default Role' value={defRole} onChange={v => setDefRole(v)} options={defRoleOpts} label='Default Role' w={98} />}
