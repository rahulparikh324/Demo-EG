...
class WorkOrderList extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      selectedTitleVal: [],
      priorityVal: null,
      statusVal: enums.workOrderStatus.slice(0, 3),
      size: 0,
  ...
  async componentDidMount() {
    ...
    if (this.getUserSitesData.isActiveSiteAllSite(active)) {
      ...
      this.setState({ companyAndSiteFilter: { companyFilter: false, siteFilter: true }, allSites })
    } else {
      this.state.loginData.usersites.forEach(({ site_id, site_name }) => site_id === active && allSites.push({ site_id, site_name }))
      this.setState({ companyAndSiteFilter: { companyFilter: false, siteFilter: false }, allSites })
    }
  ...
  fetchInitialOptions = async () => {
    ...
    const assetNameOpts = await issueFilterOptions(URL.filterIssuesAssetOptions, payload)
    const siteNameOpts = await issueFilterOptions(URL.filterIssuesSitesOptions, payload)

    this.setState({
      allTitles: this.state.primaryFilterName !== this.filterEnums.TITLE ? titleOpts.list : this.state.allTitles,
      allAssetsLists: this.state.primaryFilterName !== this.filterEnums.ASSET_NAME ? assetNameOpts.list : this.state.allAssetsLists,
      allSites: this.state.primaryFilterName !== this.filterEnums.SITE_NAME ? siteNameOpts.list : this.state.allSites,
  ...
  handleOnClickOnIssue = data => {
    if (this.isAccesibleByUser()) {
      const { id } = data
      history.push(`/issues/details/${id}`)
    }
  }
        ...
        <Grid item xs={12}>
          <Paper className={classes.paper + ' tableminheight'} elevation={0}>
            <Grid container spacing={2}>
              ...
              <Grid className='text_r' item xs={7}>
              ...
              <Grid item xs={12}>
                <div className='table-responsive dashboardtblScroll' id='style-1' style={{ maxHeight: '650px', height: '650px' }}>
                  <Table onWheel={e => this.handleScroll()} size='small' stickyHeader={true}>
                    <TableHead>
                      <TableRow>
                        <TableCell onClick={() => this.clickOnHeaderCell()} classes={{ root: classes.headRoot }} style={this.state.selectedTitleVal.length !== 0 ? { background: '#eeeeee' } : { background: '#fafafa' }} id='title' align='left' padding='normal'>
                          {'Issue Title'}
                          <FilterListIcon fontSize='small' style={{ marginLeft: '10px' }} />
                        </TableCell>
                        <TableCell onClick={() => this.clickOnHeaderCell()} classes={{ root: classes.headRoot }} style={this.state.selectedAssetNameVal.length !== 0 ? { background: '#eeeeee' } : { background: '#fafafa' }} id='name' align='left' padding='normal'>
                          {'Asset Name'}
                          <FilterListIcon fontSize='small' style={{ marginLeft: '10px' }} />
                        </TableCell>
                        <TableCell onClick={() => this.clickOnHeaderCell()} classes={{ root: classes.headRoot }} style={this.state.statusVal.length !== 0 ? { background: '#eeeeee' } : { background: '#fafafa' }} id='status' align='left' padding='normal'>
                          {'Status'}
                          <FilterListIcon fontSize='small' style={{ marginLeft: '10px' }} />
                        </TableCell>
                        <TableCell onClick={() => this.clickOnHeaderCell()} classes={{ root: classes.headRoot }} style={this.state.priorityVal !== null ? { background: '#eeeeee' } : { background: '#fafafa' }} id='priority' align='left' padding='normal'>
                          {'Priority'}
                          <FilterListIcon fontSize='small' style={{ marginLeft: '10px' }} />
                        </TableCell>
                    ...
                    {this.state.isDataLoading ? (
                      ...
                      <TableBody>
                        {rows.map((tableRow, key) => {
                          return (
                            <TableRow key={key} onClick={e => this.handleOnClickOnIssue(tableRow)} className={this.isAccesibleByUser() ? 'table-with-row-click' : ''}>
                              <TableCell className={classes.tableCell} width='25%'>
                                {tableRow.workOrderName ? tableRow.workOrderName : '-'}
                              </TableCell>
                              <TableCell className={classes.tableCell} width='30%'>
                                {tableRow.assetName ? tableRow.assetName : '-'}
                              </TableCell>
                              <TableCell className={classes.tableCell} width='10%'>
                                {tableRow.status ? tableRow.status : '-'}
                              </TableCell>
                              <TableCell className={classes.tableCell} width='10%'>
                                {tableRow.priority ? tableRow.priority : '-'}
                              </TableCell>
                              <TableCell className={classes.tableCell} width='10%'>
                                <div className='py-1'>{tableRow.timeElapsed}</div>
                              </TableCell>
                            </TableRow>
                          )
                        })}
