...
const EditSection = ({ open, onClose, editSection, reFetch }) => {
    ...
    <Modal open={open} onClose={onClose} aria-labelledby='simple-modal-title' aria-describedby='simple-modal-description'>
      <div style={modalStyle} className='add-task-modal'>
        <div className='d-flex flex-row justify-content-between align-items-center px-3 py-2' style={{ background: '#e0e0e080' }}>
          <div style={{ fontSize: '15px', fontWeight: 800, fontFamily: 'EudoxusSans-Medium' }}>Edit Section</div>
          <IconButton onClick={onClose} size='small'>
            <CloseIcon fontSize='small' />
          </IconButton>
        </div>
        <div className='px-3 py-2'>
          <MinimalInput placeholder='Enter Enter Section' label='Section' value={sectionInput} onChange={setSectionInput} error={error.sectionInput} onFocus={() => setError({ ...error, sectionInput: null })} />
        </div>
        <div className='content-bar bottom-bar mx-2'>
          <MinimalButton variant='contained' color='default' text='Cancel' onClick={onClose} />
          <MinimalButton variant='contained' color='primary' text='Update' loadingText='Updating...' onClick={validateForm} disabled={isLoading} loading={isLoading} style={{ marginLeft: '10px' }} />
        </div>
      </div>
    </Modal>
  )
