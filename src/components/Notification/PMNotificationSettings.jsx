...
import { NotificationSettingConfig, ErrorDiv } from './components'
import Button from '@material-ui/core/Button'
import updatePMNotificationConfig from '../../Services/updatePMNotification'
import { Toast } from '../../Snackbar/useToast'
import DialogPrompt from '../DialogPrompt'

function PMNotificationSettings({ enabled, config }) {
  const [firstNotification, setFirstNotification] = useState(config.first_reminder_before_on_status === 1 ? true : false)
  ...
  const [secondNotification, setSecondNotification] = useState(config.second_reminder_before_on_status === 1 ? true : false)
  ...
  const [firstNotificationTime, setFirstNotificationTime] = useState(config.first_reminder_before_on)
  const [firstNotificationSpan, setFirstNotificationSpan] = useState(config.first_reminder_before_on_type === 29 ? 2 : config.first_reminder_before_on_type === 39 ? 1 : 0)
  const [secondNotificationSpan, setSecondNotificationSpan] = useState(config.second_reminder_before_on_type === 29 ? 2 : config.second_reminder_before_on_type === 39 ? 1 : 0)
  const [firstNotificationMH, setFirstNotificationMH] = useState(config.first_reminder_before_on_meter_hours)
  const [secondNotificationMH, setSecondNotificationMH] = useState(config.second_reminder_before_on_meter_hours)
  const [secondNotificationMHMAX, setSecondNotificationMHMAX] = useState(0)
  const [secondNotificationTime, setSecondNotificationTime] = useState(config.second_reminder_before_on)
    ...
    <div>
      ...
      {!enabled && (
        <div className='disabled-settings d-flex flex-row justify-content-center align-items-center'>
          <div className='title'> Please enable PM Notifications in order to configure the settings</div>
        </div>
      )}
      {updating && (
        <div className='disabled-settings d-flex flex-row justify-content-center align-items-center'>
...
export default PMNotificationSettings
