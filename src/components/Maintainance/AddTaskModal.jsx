...
function AddTaskModal({ open, handleClose, addTasks, prevTasks }) {
  ...
  const addTasksToPM = () => {
    ...
    const tasksToAdd = loadedTasks.filter(task => arr.includes(task.task_id))
    const arrx = [...prevTasks, ...tasksToAdd]
    const uni = [...new Set(arrx.map(s => JSON.stringify(s)))].map(k => JSON.parse(k))
    addTasks(uni)
    handleClose()
  }
  //
  ...
  const body = (
    <div style={modalStyle} className='add-task-modal'>
      ...
      <div className='content-bar bottom-bar'>
        ...
        <Button variant='contained' color='primary' className='nf-buttons' disableElevation style={{ marginLeft: '10px' }} onClick={addTasksToPM}>
          Add to PM
        </Button>
      </div>
    </div>
